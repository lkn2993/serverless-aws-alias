...
module.exports = {
	...
	createAlias() {
		...
		const params = {
			...
			Parameters: [],
			TemplateBody: JSON.stringify(this._serverless.service.provider.compiledCloudFormationAliasCreateTemplate).replace(/Delete/g, 'Retain'),
	...
	writeAliasTemplateToDisk() {
		...
		this._serverless.utils.writeFileSync(cfTemplateFilePath,
			JSON.parse(JSON.stringify(this._serverless.service.provider.compiledCloudFormationAliasCreateTemplate).replace(/Delete/g, 'Retain')));
		const x = JSON.stringify(this._serverless.service.provider.compiledCloudFormationAliasCreateTemplate)
		return BbPromise.resolve();
	}
