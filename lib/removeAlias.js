...
module.exports = {
	...
	removeAlias(currentTemplate, aliasStackTemplates, currentAliasStackTemplate) {
		...
		if (_.isEmpty(currentAliasStackTemplate)) {
			throw new this._serverless.classes.Error(`Internal error: Stack for master alias is not deployed. Try to solve the problem by manual interaction with the AWS console.`);
		}

		// We're ready for removal
		this._serverless.cli.log(`Removing master alias and stage ...`);
